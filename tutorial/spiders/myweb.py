...
filters_pages=r'.*?(facebook|twitter|linkedin|youtube|yahoo|Flickr|apple|google|vimeo|instagram|cie|ibo|BBC|news|newsletter|calendar|job).*?'
...
class myspider(scrapy.Spider):
    ...
    def parse_web(self, response):
        filter_exclude=r'(?i)Newsletter|Calendar|summary|music|policy|Mr|Mrs|news|blog|Curriculum|summer|jobs|contact|arts'
        ...
        body_exclude=response.xpath('//body//text()').re(filter_exclude)
        if (len(body_exclude)==0) and (len(title)>0 or len(body)>0):
